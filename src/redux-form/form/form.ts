...
const createFormControl = function <
    T,
...
>({
    ...
    const controlName = `${formName}/${name}` as ControlName;
    const control = createControl(controlName, {
        value,
        validateOnChange,
        validators,
    });
    ...
    return Object.assign(control, {
        selectors: {
            getValue: (state: State) => state[formName][controlName].value,
            getErrors: (state: State) => state[formName][controlName].errors,
            getDirty: (state: State) => state[formName][controlName].dirty,
        },
    });
};
...
>(name: string, config: T): Form<T, K> {
    ...
    for (const [key, controlConfig] of Object.entries(config)) {
        const controlName = `${name}/${key}` as K;
            ...
            [name]: formReducer as Reducer<
                CombinedState<
                    Record<
                        K,
                        ControlState<
                            FormControlType<FormControlFromConfig<T[K]>>
                        >
                    >
                >,
                AnyAction
